...
describe('GET  /api/classroom/getclassrooms', () => {
    ...
    //Get Class Room Table For Admin.
    it('Should return 200 Allowed To See Class Room For Admin', async () => {
        require('../utils/verifyUser').verifyToken.mockImplementation((req, res, next) => {
            req.user = { id: adminId, isAdmin: true }
            next()
        })
        const res = await req(app).get('/api/classroom/getclassrooms');
        expect(res.statusCode).toBe(200);
    }, 20000); 

     //Get Class Room For Faculty.
    it('Should return 200 Allowed To See Class Room For Faculty', async () => {
        require('../utils/verifyUser').verifyToken.mockImplementation((req, res, next) => {
    ...
    it('Should return 200 Allowed To See Class Room For Student', async () => {
        require('../utils/verifyUser').verifyToken.mockImplementation((req, res, next) => {
