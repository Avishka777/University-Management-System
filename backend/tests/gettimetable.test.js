...
describe('GET /api/timetable/gettimetables', () => {
    ...
    //Get Time Table For Admin.
    it('Should return 200 Allowed To See Time Table For Admin', async () => {
        require('../utils/verifyUser').verifyToken.mockImplementation((req, res, next) => {
            req.user = { id: adminId, isAdmin: true }
            next()
        })
        const res = await req(app).get('/api/timetable/gettimetables');
        expect(res.statusCode).toBe(200);
    }, 20000); 

     //Get Time Table For Faculty.
    it('Should return 200 Allowed To See Time Table For Faculty', async () => {
        require('../utils/verifyUser').verifyToken.mockImplementation((req, res, next) => {
    ...
    it('Should return 200 Allowed To See Time Table For Student', async () => {
        require('../utils/verifyUser').verifyToken.mockImplementation((req, res, next) => {
